...
def build_filter_complex(params: RenderParams, vW: int, vH: int) -> str:
    ...
    if def_["mode"] == "dual":
        ...
        # Fondo: relleno sólido (antes era el video en cover+blur+oscurecido,
        # pero eso era el paso más caro del render por lejos — procesaba los
        # 1080x1920 píxeles del canvas completo con un gaussian blur real en
        # cada frame. En un servidor de 512MB/CPU compartida esto dominaba
        # el tiempo de render. Se prioriza velocidad acá; si en algún
        # momento se quiere recuperar el efecto de fondo difuminado, la
        # forma barata de hacerlo es escalar el video a una resolución bien
        # chica (ej. 96x170), aplicar blur ahí, y escalar ese resultado de
        # vuelta a 1080x1920 — el costo del blur cae en proporción al área
        # procesada, y a esa escala el resultado visual es indistinguible.
        filters.append(f"color=c=0x0d1117:s={W}x{H}:d=1[bg]")
...
def render_clip(params: RenderParams, output_path: str) -> str:
    ...
    try:
        ...
        # -threads 1 evita picos de RAM de ffmpeg en servidores con poca
        # memoria (ej. el free tier de Render, 512MB) — sin esto, ffmpeg
        # intenta paralelizar y cada thread reserva sus propios buffers de
        # frame, lo cual puede tirar el proceso por out-of-memory.
