"""
Renderizado de clips en el servidor con ffmpeg optimizado para CPU limitada (Render Free Tier).
"""
import json
import os
import subprocess
...
def build_filter_complex(params: RenderParams, vW: int, vH: int) -> Tuple[str, List[str]]:
    ...
    if def_["mode"] == "dual":
        ...
        # ⚡ BLUR ULTRA-OPTIMIZADO (135x240 px = Cero consumo de CPU)
        filters.append(
            f"[0:v]scale=135:240:force_original_aspect_ratio=increase,"
            f"crop=135:240,"
            f"boxblur=5:1,"
            f"scale={W}:{H},"
...
def render_clip(params: RenderParams, output_path: str) -> str:
    ...
    try:
        ...
        cmd += [
            ...
            "-map", "[vout]", "-map", "0:a?",
            "-r", "30",                 # ⚡ FORZAR 30 FPS (Elimina el 50% del trabajo de CPU)
            "-c:v", "libx264",
            "-preset", "ultrafast",
            "-tune", "zerolatency",     # ⚡ AJUSTE RÁPIDO DE X264
            "-crf", "26",               # ⚡ REDUCE LA CARGA DE COMPRESIÓN
            "-c:a", "aac", "-b:a", "96k",
            "-movflags", "+faststart",
            output_path,
        ]
